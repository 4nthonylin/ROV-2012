void ps2(){
  ...
  if(error == 0){
    ...
    if(ps2x.Button(PSB_PAD_LEFT))  servo[1]-= 2;
    if(ps2x.Button(PSB_PAD_RIGHT)) servo[1]+= 2;
    if(ps2x.Button(PSB_PAD_UP))    servo[0]-= 2;
    if(ps2x.Button(PSB_PAD_DOWN))  servo[0]+= 2;
    if(ps2x.Button(PSB_PINK))      servo[2]-= 1;
    if(ps2x.Button(PSB_RED))       servo[2]+= 1;
    if(ps2x.Button(PSB_R1))        servo[3] = 180;
    else if(ps2x.Button(PSB_R2))   servo[3] = 0;
    else                           servo[3] = 90;

    if(ps2x.Button(PSB_BLUE)) pwm[4] = 100;
    if(ps2x.Button(PSB_GREEN)) pwm[4] = 0;
    if(ps2x.Button(PSB_SELECT)) for(int index = 0; index < 4; index++) servo[index] = 90;
  }
  else{
